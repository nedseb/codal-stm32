...
{
    if (HCI.leCreateConn(0x0060, 0x0030, 0x00, peerBdaddrType, peerBdaddr, _ownBdaddrType, 0x0006, 0x000c, 0x0000,
                         0x00c8, 0x0004, 0x0006) != 0) {
        return false;
    }
...
void ATTClass::addConnection(uint16_t handle, uint8_t role, uint8_t peerBdaddrType, uint8_t peerBdaddr[6],
                             uint16_t /*interval*/, uint16_t /*latency*/, uint16_t /*supervisionTimeout*/,
...
void ATTClass::findInfoReq(uint16_t connectionHandle, uint16_t mtu, uint8_t dlen, uint8_t data[])
{
    ...
    for (uint16_t i = (findInfoReq->startHandle - 1); i < GATT.attributeCount() && i <= (findInfoReq->endHandle - 1);
         i++) {
        BLELocalAttribute* attribute = GATT.attribute(i);
        uint16_t handle              = (i + 1);
        bool isValueHandle           = (attribute->type() == BLETypeCharacteristic) &&
                             (((BLELocalCharacteristic*)attribute)->valueHandle() == handle);
...
void ATTClass::findByTypeReq(uint16_t connectionHandle, uint16_t mtu, uint8_t dlen, uint8_t data[])
{
    ...
    if (findByTypeReq->type == BLETypeService) {
        for (uint16_t i = (findByTypeReq->startHandle - 1);
             i < GATT.attributeCount() && i <= (findByTypeReq->endHandle - 1); i++) {
            BLELocalAttribute* attribute = GATT.attribute(i);

            if ((attribute->type() == findByTypeReq->type) && (attribute->uuidLength() == valueLength) &&
                memcmp(attribute->uuidData(), value, valueLength) == 0) {
...
void ATTClass::readByGroupReq(uint16_t connectionHandle, uint16_t mtu, uint8_t dlen, uint8_t data[])
{
    ...
    for (uint16_t i = (readByGroupReq->startHandle - 1);
         i < GATT.attributeCount() && i <= (readByGroupReq->endHandle - 1); i++) {
        BLELocalAttribute* attribute = GATT.attribute(i);
...
int ATTClass::readByGroupReq(uint16_t connectionHandle, uint16_t startHandle, uint16_t endHandle, uint16_t uuid,
...
void ATTClass::readOrReadBlobReq(uint16_t connectionHandle, uint16_t mtu, uint8_t opcode, uint8_t dlen, uint8_t data[])
{
    ...
    if (attributeType == BLETypeService) {
        ...
        if (characteristic->handle() == handle) {
            ...
            for (int i = 0; i < ATT_MAX_PEERS; i++) {
                if (_peers[i].connectionHandle == connectionHandle) {
                    characteristic->readValue(BLEDevice(_peers[i].addressType, _peers[i].address), offset,
                                              &response[responseLength], valueLength);
...
void ATTClass::readByTypeReq(uint16_t connectionHandle, uint16_t mtu, uint8_t dlen, uint8_t data[])
{
    ...
    for (uint16_t i = (readByTypeReq->startHandle - 1);
         i < GATT.attributeCount() && i <= (readByTypeReq->endHandle - 1); i++) {
        BLELocalAttribute* attribute = GATT.attribute(i);
        uint16_t handle              = (i + 1);
        ...
        else if (attribute->type() == BLETypeCharacteristic && attribute->uuidLength() == 2 &&
                 memcmp(&readByTypeReq->uuid, attribute->uuidData(), 2) == 0) {
            BLELocalCharacteristic* characteristic = (BLELocalCharacteristic*)attribute;
...
int ATTClass::readByTypeReq(uint16_t connectionHandle, uint16_t startHandle, uint16_t endHandle, uint16_t type,
                            uint8_t responseBuffer[])
{
    struct __attribute__((packed)) {
        uint8_t op;
...
void ATTClass::writeReqOrCmd(uint16_t connectionHandle, uint16_t mtu, uint8_t op, uint8_t dlen, uint8_t data[])
{
    ...
    if (attribute->type() == BLETypeCharacteristic) {
        ...
        if (handle != characteristic->valueHandle() || withResponse
                ? ((characteristic->properties() & BLEWrite) == 0)
        ...
        for (int i = 0; i < ATT_MAX_PEERS; i++) {
            if (_peers[i].connectionHandle == connectionHandle) {
                characteristic->writeCccdValue(BLEDevice(_peers[i].addressType, _peers[i].address),
                                               *((uint16_t*)value));
                break;
            }
        }
...
void ATTClass::execWriteReq(uint16_t connectionHandle, uint16_t mtu, uint8_t dlen, uint8_t data[])
{
    ...
    if (_longWriteHandle && (flag & 0x01)) {
        ...
        for (int i = 0; i < ATT_MAX_PEERS; i++) {
            if (_peers[i].connectionHandle == connectionHandle) {
                characteristic->writeValue(BLEDevice(_peers[i].addressType, _peers[i].address), _longWriteValue,
                                           _longWriteValueLength);
                break;
            }
        }
    }
            ...
            for (int i = 2; i < respLength; i += lengthPerService) {
                ...
                if (serviceUuidFilter == NULL ||
                    ...
                    BLERemoteService* service =
                        new BLERemoteService(rawService->uuid, uuidLen, rawService->startHandle, rawService->endHandle);
...
bool ATTClass::discoverCharacteristics(uint16_t connectionHandle, BLERemoteDevice* device)
{
    ...
    for (int i = 0; i < serviceCount; i++) {
        ...
        while (1) {
            int respLength =
                readByTypeReq(connectionHandle, reqStartHandle, reqEndHandle, BLETypeCharacteristic, responseBuffer);
            ...
            if (responseBuffer[0] == ATT_OP_READ_BY_TYPE_RESP) {
                ...
                for (int i = 2; i < respLength; i += lengthPerCharacteristic) {
                    ...
                    BLERemoteCharacteristic* characteristic = new BLERemoteCharacteristic(
                        rawCharacteristic->uuid, uuidLen, connectionHandle, rawCharacteristic->startHandle,
                        rawCharacteristic->properties, rawCharacteristic->valueHandle);
...
bool ATTClass::discoverDescriptors(uint16_t connectionHandle, BLERemoteDevice* device)
{
    ...
    for (int i = 0; i < serviceCount; i++) {
        ...
        for (int j = 0; j < characteristicCount; j++) {
            ...
            BLERemoteCharacteristic* nextCharacteristic =
                (j == (characteristicCount - 1)) ? NULL : service->characteristic(j + 1);
            ...
            while (1) {
                ...
                if (responseBuffer[0] == ATT_OP_FIND_INFO_RESP) {
                    ...
                    for (int i = 2; i < respLength; i += lengthPerDescriptor) {
                        ...
                        BLERemoteDescriptor* descriptor = new BLERemoteDescriptor(
                            rawDescriptor->uuid, uuidLen, connectionHandle, rawDescriptor->handle);
...
int ATTClass::writeReq(uint16_t connectionHandle, uint16_t handle, const uint8_t* data, uint8_t dataLen,
                       uint8_t responseBuffer[])
{
    struct __attribute__((packed)) {
        uint8_t op;
