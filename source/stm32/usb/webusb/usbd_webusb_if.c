...
__ALIGN_BEGIN uint8_t USBD_WebUSB_BOSDesc[USB_WEBUSB_SIZ_BOS_DESC] __ALIGN_END = {
    ...
    USB_WEBUSB_SIZ_BOS_DESC,
    0x00, /* Total length of BOS descriptor and all of its sub descs */
...
static int8_t USBD_WebUSB_Control(uint8_t cmd, uint8_t* pbuf, uint16_t length)
{
    ...
    switch (cmd) {
        ...
        case WebUSB_SET_LINE_CODING:
            linecoding_webusb.bitrate    = (uint32_t)(pbuf[0] | (pbuf[1] << 8) | (pbuf[2] << 16) | (pbuf[3] << 24));
        ...
        case WebUSB_SET_CONTROL_LINE_STATE:
            lineState_webusb = (((USBD_SetupReqTypedef*)pbuf)->wValue & 0x01) != 0;  // Check DTR state
...
bool webUSB_connected()
{
    ...
    return ((hUSBD_Device_WebUSB.dev_state == USBD_STATE_CONFIGURED) && (transmitTime < USB_WebUSB_TRANSMIT_TIMEOUT) &&
            lineState_webusb);
}
