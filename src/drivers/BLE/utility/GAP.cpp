...
GAPClass::GAPClass()
    : _advertising(false), _scanning(false), _advertisingInterval(160), _connectable(true), _discoverEventHandler(NULL)
{
}
...
int GAPClass::scan(bool withDuplicates)
{
    ...
    // active scan, 10 ms scan interval (N * 0.625), 10 ms scan window (N * 0.625), public or static random own address
    // type, no filter
...
void GAPClass::handleLeAdvertisingReport(uint8_t type, uint8_t addressType, uint8_t address[6], uint8_t eirLength,
                                         uint8_t eirData[], int8_t rssi)
{
    if (!_scanning) {
        return;
    }
