...
const PinAnalogSwitch PinMapAnalogSwitch[] = {{PA_0, LL_SYSCFG_ANALOG_SWITCH_PA0},
...
/* Map STM_PIN to LL */
const uint32_t pin_map_ll[16] = {LL_GPIO_PIN_0,  LL_GPIO_PIN_1,  LL_GPIO_PIN_2,  LL_GPIO_PIN_3,
...
void configure_dualpad_switch(PinName pin, int function)
{
    ...
    while (AnalogSwitch->pin != NC) {
        ...
        if ((AnalogSwitch->pin | PDUAL) == (pin | PDUAL)) {
            if (((function & STM_MODE_ANALOG) != STM_MODE_ANALOG) && ((pin & PDUAL) == PDUAL)) {
                /**
