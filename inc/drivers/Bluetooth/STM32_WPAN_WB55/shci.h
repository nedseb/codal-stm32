...
typedef PACKED_STRUCT
{
     ...
     * For each characteristic, the number of attribute records goes from two to five depending on the characteristic
     * properties:
     ...
     * The total calculated value must be increased by 9, due to the records related to the standard attribute profile
     * and GAP service characteristics, and automatically added when initializing GATT and GAP layers
    ...
    /**
     * NumAttrServ
     * Defines the maximum number of services that can be stored in the GATT database. Note that the GAP and GATT
     * services are automatically added at initialization so this parameter must be the number of user services
     * increased by two.
    ...
    /**
     * AttrValueArrSize
     * NOTE: This parameter is ignored by the CPU2 when the parameter "Options" is set to "LL_only" ( see Options
    ...
    /**
     * MblockCount
     * NOTE: This parameter is overwritten by the CPU2 with an hardcoded optimal value when the parameter "Options" is
     * set to "LL_only" ( see Options description in that structure )
     *
    ...
    /**
     * CentralSca
     * The sleep clock accuracy handled in Central mode. It is used to determine the connection and advertising events
     * timing. It is transmitted to the slave in CONNEC_REQ PDU used by the slave to calculate the window widening, see
     * PeripheralSca and Bluetooth Core Specification v5.0 Vol 6 - Part B - chap 4.5.7 and 4.2.2 Possible values:
     ...
     * - bit 0:   1: Calibration for the RF system wakeup clock source   0: No calibration for the RF system wakeup
     * clock source
     * - bit 1:   1: STM32W5M Module device                              0: Other devices as STM32WBxx SOC, STM32WB1M
     * module
    ...
    /**
     * MaxConnEventLength
     * This parameter determines the maximum duration of a slave connection event. When this duration is reached the
     * slave closes the current connections event (whatever is the CE_length parameter specified by the master in
     * HCI_CREATE_CONNECTION HCI command), expressed in units of 625/256 us (~2.44 us)
     *    - Min value: 0 (if 0 is specified, the master and slave perform only a single TX-RX exchange per connection
     * event)
     *    - Max value: 1638400 (4000 ms). A higher value can be specified (max 0xFFFFFFFF) but results in a maximum
     * connection time of 4000 ms as specified. In this case the parameter is not applied, and the predicted CE length
     * calculated on slave is not shortened
     */
     ...
     *    - Min value: 0
     *    - Max value:  820 (~2 ms). A higher value can be specified, but the value that implemented in stack is forced
     * to ~2 ms
     */
     ...
     * This parameter is linked to max_adv_data_len such as both compliant with allocated Total memory computed with
     * BLE_EXT_ADV_BUFFER_SIZE based on Max Extended advertising configuration supported. This parameter is considered
     * by the CPU2 when Options has SHCI_C2_BLE_INIT_OPTIONS_EXT_ADV flag set
     */
    uint8_t max_adv_set_nbr;

    /* Maximum advertising data length (in bytes)
     * Range: 31 .. 1650 with limitation:
     * This parameter is linked to max_adv_set_nbr such as both compliant with allocated Total memory computed with
     * BLE_EXT_ADV_BUFFER_SIZE based on Max Extended advertising configuration supported. This parameter is considered
     * by the CPU2 when Options has SHCI_C2_BLE_INIT_OPTIONS_EXT_ADV flag set
     */
...
/** No response parameters*/
    ...
    #define SHCI_OPCODE_C2_CONCURRENT_GET_NEXT_BLE_EVT_TIME \
        ((SHCI_OGF << 10) + SHCI_OCF_C2_CONCURRENT_GET_NEXT_BLE_EVT_TIME)
/** command parameters */
typedef PACKED_STRUCT
{
...
/** No response parameters*/
    ...
    #define SHCI_OPCODE_C2_CONCURRENT_ENABLE_NEXT_802154_EVT_NOTIFICATION \
        ((SHCI_OGF << 10) + SHCI_OCF_C2_CONCURRENT_ENABLE_NEXT_802154_EVT_NOTIFICATION)
/** No command parameters */
/** No response parameters*/
...
typedef enum {
     ...
     * The buffer shall be allocated in SRAM2
     */
    #define THREAD_NVM_SRAM_SIZE (1'016)
 ...
 *               When the wireless firmware receives that command, it responds SHCI_FUS_CMD_NOT_SUPPORTED the first
 * time. When the wireless firmware receives that command a second time, it reboots the full device with the FUS running
 * on CPU2
 *
 ...
 *                -  FLASH_ACTIVITY_CONTROL_PES : The CPU2 set the PES bit to prevent the CPU1 to either read or write
 * in flash
 *                -  FLASH_ACTIVITY_CONTROL_SEM7 : The CPU2 gets the semaphore 7 to prevent the CPU1 to either read or
 * write in flash. This requires the CPU1 to first get semaphore 7 before erasing or writing the flash.
 *
 * @retval Status
 */
 ...
 *                -  SET_SYSTEM_CLOCK_HSE_TO_PLL : CPU2 set system clock to PLL, PLL must be configured and started
 * before.
 *                -  SET_SYSTEM_CLOCK_PLL_ON_TO_HSE : CPU2 set System clock to HSE, PLL is still ON after command
 * execution.
 *                -  SET_SYSTEM_CLOCK_PLL_OFF_TO_HSE : CPU2 set System clock to HSE, PLL is turned OFF after command
 * execution.
 *
 * @retval Status
 */
