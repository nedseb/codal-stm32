...
LSM6DSL_Accelerometer::LSM6DSL_Accelerometer(STM32I2C* i2c, uint8_t address, CoordinateSpace& coordinateSpace,
                                             uint16_t id)
...
{
    if (value <= 2)
        return LSM6DSL_ACC_GYRO_FS_XL_2g;
    else if (value <= 4)
        return LSM6DSL_ACC_GYRO_FS_XL_4g;
    else if (value <= 8)
        return LSM6DSL_ACC_GYRO_FS_XL_8g;
    else
        return LSM6DSL_ACC_GYRO_FS_XL_16g;
}
...
LSM6DSL_ACC_GYRO_ODR_XL_t LSM6DSL_Accelerometer::getBestODR(float ms)
{
    ...
    if (freq <= 12.5)
        ...
        return LSM6DSL_ACC_GYRO_ODR_XL_26Hz;
    else if (freq <= 52)
        return LSM6DSL_ACC_GYRO_ODR_XL_52Hz;
    else if (freq <= 104)
        return LSM6DSL_ACC_GYRO_ODR_XL_104Hz;
    else if (freq <= 208)
        return LSM6DSL_ACC_GYRO_ODR_XL_208Hz;
    else if (freq <= 416)
