...
static uint8_t  USBD_WebUSB_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  if (pdev->dev_speed == USBD_SPEED_HIGH) {
    ...
    (void)USBD_LL_OpenEP(pdev, CDC_IN_EP, USBD_EP_TYPE_BULK,
                         WebUSB_DATA_HS_IN_PACKET_SIZE);

    pdev->ep_in[CDC_IN_EP & 0xFU].is_used = 1U;

    /* Open EP OUT */
    (void)USBD_LL_OpenEP(pdev, CDC_OUT_EP, USBD_EP_TYPE_BULK,
                         WebUSB_DATA_HS_OUT_PACKET_SIZE);
    ...
    /* Open EP IN */
    (void)USBD_LL_OpenEP(pdev, CDC_IN_EP, USBD_EP_TYPE_BULK,
                         WebUSB_DATA_FS_IN_PACKET_SIZE);

    pdev->ep_in[CDC_IN_EP & 0xFU].is_used = 1U;

    /* Open EP OUT */
    (void)USBD_LL_OpenEP(pdev, CDC_OUT_EP, USBD_EP_TYPE_BULK,
                         WebUSB_DATA_FS_OUT_PACKET_SIZE);
    ...
    case USB_REQ_TYPE_CLASS:
      if (req->wLength != 0U) {
        if ((req->bmRequest & 0x80U) != 0U) {
          ((USBD_WebUSB_ItfTypeDef *)pdev->pUserData)->Control(req->bRequest,
                                                            (uint8_t *)hcdc->data,
                                                            req->wLength);
...
static uint8_t  USBD_WebUSB_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
  ...
  if ((pdev->pUserData != NULL) && (hcdc->CmdOpCode != 0xFFU)) {
    ((USBD_WebUSB_ItfTypeDef *)pdev->pUserData)->Control(hcdc->CmdOpCode,
                                                      (uint8_t *)hcdc->data,
