...
ALIGN(4) static uint8_t CsBuffer[sizeof(TL_PacketHeader_t) + TL_EVT_HDR_SIZE + sizeof(TL_CsEvt_t)];
...
void HW_IPCC_BLE_RxEvtNot(void)
{
    ...
    while (LST_is_empty(&EvtQueue) == FALSE) {
        ...
        if (((phcievt->evtserial.evt.evtcode) == TL_BLEEVT_CS_OPCODE) ||
            ((phcievt->evtserial.evt.evtcode) == TL_BLEEVT_CC_OPCODE)) {
    ...
    switch (packet_type) {
        case TL_MB_MM_RELEASE_BUFFER:
            ...
            switch (p_evt_packet->evtserial.evt.evtcode) {
                ...
                case TL_BLEEVT_VS_OPCODE:
                    TL_MM_DBG_MSG("mm evt released: 0x%02X", p_evt_packet->evtserial.evt.evtcode);
                    TL_MM_DBG_MSG(" subevtcode: 0x%04X",
                                  ((TL_AsynchEvt_t*)(p_evt_packet->evtserial.evt.payload))->subevtcode);
                    TL_MM_DBG_MSG(" buffer addr: 0x%08X", p_evt_packet);
                    break;
        ...
        case TL_MB_BLE_CMD_RSP:
            ...
            switch (p_evt_packet->evtserial.evt.evtcode) {
                case TL_BLEEVT_CS_OPCODE:
                    ...
                    TL_HCI_CMD_DBG_MSG(" cmd opcode: 0x%04X",
                                       ((TL_CsEvt_t*)(p_evt_packet->evtserial.evt.payload))->cmdcode);
                ...
                case TL_BLEEVT_CC_OPCODE:
                    TL_HCI_CMD_DBG_MSG("ble rsp: 0x%02X", p_evt_packet->evtserial.evt.evtcode);
                    TL_HCI_CMD_DBG_MSG(" cmd opcode: 0x%04X",
                                       ((TL_CcEvt_t*)(p_evt_packet->evtserial.evt.payload))->cmdcode);
                    TL_HCI_CMD_DBG_MSG(" numhci: 0x%02X", ((TL_CcEvt_t*)(p_evt_packet->evtserial.evt.payload))->numcmd);
                    TL_HCI_CMD_DBG_MSG(" status: 0x%02X",
                                       ((TL_CcEvt_t*)(p_evt_packet->evtserial.evt.payload))->payload[0]);
                    if ((p_evt_packet->evtserial.evt.plen - 4) != 0) {
                        TL_HCI_CMD_DBG_MSG(" payload:");
                        TL_HCI_CMD_DBG_BUF(&((TL_CcEvt_t*)(p_evt_packet->evtserial.evt.payload))->payload[1],
                                           p_evt_packet->evtserial.evt.plen - 4, "");
                    }
                    break;
        ...
        case TL_MB_BLE_ASYNCH_EVT:
            ...
            if (p_evt_packet->evtserial.evt.evtcode != TL_BLEEVT_VS_OPCODE) {
                ...
                TL_HCI_EVT_DBG_MSG("ble evt: 0x%02X", p_evt_packet->evtserial.evt.evtcode);
                TL_HCI_EVT_DBG_MSG(" subevtcode: 0x%04X",
                                   ((TL_AsynchEvt_t*)(p_evt_packet->evtserial.evt.payload))->subevtcode);
                if ((p_evt_packet->evtserial.evt.plen - 2) != 0) {
                    TL_HCI_EVT_DBG_MSG(" payload:");
                    TL_HCI_EVT_DBG_BUF(((TL_AsynchEvt_t*)(p_evt_packet->evtserial.evt.payload))->payload,
                                       p_evt_packet->evtserial.evt.plen - 2, "");
                }
            }
        ...
        case TL_MB_SYS_CMD_RSP:
            ...
            switch (p_evt_packet->evtserial.evt.evtcode) {
                case TL_BLEEVT_CC_OPCODE:
                    ...
                    TL_SHCI_CMD_DBG_MSG(" cmd opcode: 0x%02X",
                                        ((TL_CcEvt_t*)(p_evt_packet->evtserial.evt.payload))->cmdcode);
                    TL_SHCI_CMD_DBG_MSG(" status: 0x%02X",
                                        ((TL_CcEvt_t*)(p_evt_packet->evtserial.evt.payload))->payload[0]);
                    if ((p_evt_packet->evtserial.evt.plen - 4) != 0) {
                        TL_SHCI_CMD_DBG_MSG(" payload:");
                        TL_SHCI_CMD_DBG_BUF(&((TL_CcEvt_t*)(p_evt_packet->evtserial.evt.payload))->payload[1],
                                            p_evt_packet->evtserial.evt.plen - 4, "");
                    }
                    break;
        ...
        case TL_MB_SYS_ASYNCH_EVT:
            ...
            if (p_evt_packet->evtserial.evt.evtcode != TL_BLEEVT_VS_OPCODE) {
                ...
                TL_SHCI_EVT_DBG_MSG(" subevtcode: 0x%04X",
                                    ((TL_AsynchEvt_t*)(p_evt_packet->evtserial.evt.payload))->subevtcode);
                if ((p_evt_packet->evtserial.evt.plen - 2) != 0) {
                    TL_SHCI_EVT_DBG_MSG(" payload:");
                    TL_SHCI_EVT_DBG_BUF(((TL_AsynchEvt_t*)(p_evt_packet->evtserial.evt.payload))->payload,
                                        p_evt_packet->evtserial.evt.plen - 2, "");
                }
            }
