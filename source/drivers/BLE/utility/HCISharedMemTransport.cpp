...
{
    /**
     * The current implementation assumes the taskGUI will not send a new HCI ACL DATA packet before this ack is
     * received ( which means the CPU2 has handled the previous packet ) In order to implement a secure mechanism, it is
     * required either
        ...
        case TL_BLEEVT_PKT_TYPE: {
            ...
            if (phase_running == false) {
                ...
                if ((hcievt->evtserial.evt.evtcode == TL_BLEEVT_CC_OPCODE) &&
                    (hcievt->evtserial.evt.payload[0] == 0x01) && (hcievt->evtserial.evt.payload[1] == 0x0C) &&
                ...
                if ((hcievt->evtserial.evt.evtcode == TL_BLEEVT_CC_OPCODE) &&
                    (hcievt->evtserial.evt.payload[0] == 0x01) && (hcievt->evtserial.evt.payload[1] == 0x0F) &&
                    (hcievt->evtserial.evt.payload[2] == 0xFC)) {
                    phase_tx_power = true;
                    if (hcievt->evtserial.evt.payload[3] != 0) {
#if defined(PRINT_IPCC_INFO)
                ...
                if ((hcievt->evtserial.evt.evtcode == TL_BLEEVT_CC_OPCODE) &&
                    (hcievt->evtserial.evt.payload[0] == 0x01) && (hcievt->evtserial.evt.payload[1] == 0x01) &&
                ...
                if ((hcievt->evtserial.evt.evtcode == TL_BLEEVT_CC_OPCODE) &&
                    (hcievt->evtserial.evt.payload[0] == 0x01) && (hcievt->evtserial.evt.payload[1] == 0x8A) &&
                    (hcievt->evtserial.evt.payload[2] == 0xFC)) {
                    phase_gap_init = true;
                    if (hcievt->evtserial.evt.payload[3] != 0) {
#if defined(PRINT_IPCC_INFO)
                ...
                if ((hcievt->evtserial.evt.evtcode == TL_BLEEVT_CC_OPCODE) &&
                    (hcievt->evtserial.evt.payload[0] == 0x01) && (hcievt->evtserial.evt.payload[1] == 0x0D) &&
                    (hcievt->evtserial.evt.payload[2] == 0xFC)) {
                    if (hcievt->evtserial.evt.payload[3] != 0) {
#if defined(PRINT_IPCC_INFO)
                        printf("Error: wrong Random Addr\r\n");
                ...
                if ((hcievt->evtserial.evt.evtcode == TL_BLEEVT_CC_OPCODE) &&
                    (hcievt->evtserial.evt.payload[0] == 0x01) && (hcievt->evtserial.evt.payload[1] == 0x03) &&
...
#if defined(PRINT_IPCC_INFO)
        printf("WB copro FW version = %d.%d.%d\r\n", p_wireless_info->VersionMajor, p_wireless_info->VersionMinor,
