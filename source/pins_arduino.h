...
#define digitalPinToPinName(p)                                                                                         \
    ((((uint32_t)(p)&PNUM_MASK) < NUM_DIGITAL_PINS) ? (PinName)(digitalPin[(uint32_t)(p)&PNUM_MASK] | ((p)&ALTX_MASK)) \
     : (((uint32_t)(p)&PNUM_ANALOG_BASE) == PNUM_ANALOG_BASE) &&                                                       \
             (((uint32_t)(p)&PNUM_MASK) < NUM_ANALOG_INTERNAL_FIRST)                                                   \
         ? (PinName)(digitalPin[analogInputPin[(p)&PNUM_ANALOG_INDEX]] | ((p)&ALTX_MASK))                              \
         : NC)
#else
#define digitalPinToPinName(p)                                                                                         \
    ((((uint32_t)(p)&PNUM_MASK) < NUM_DIGITAL_PINS) ? (PinName)(digitalPin[(uint32_t)(p)&PNUM_MASK] | ((p)&ALTX_MASK)) \
...
#define analogInputToDigitalPin(p)                                             \
    ((((uint32_t)(p)&PNUM_MASK) < NUM_ANALOG_INPUTS)                           \
         ? analogInputPin[(uint32_t)(p)&PNUM_MASK] | ((uint32_t)(p)&ALTX_MASK) \
     : (((uint32_t)(p)&PNUM_ANALOG_BASE) == PNUM_ANALOG_BASE) &&               \
             (((uint32_t)(p)&PNUM_MASK) < NUM_ANALOG_INTERNAL_FIRST)           \
         ? analogInputPin[(p)&PNUM_ANALOG_INDEX] | ((uint32_t)(p)&ALTX_MASK)   \
         : (uint32_t)NC)
...
#define digitalPinHasI2C(p) \
    (pin_in_pinmap(digitalPinToPinName(p), PinMap_I2C_SDA) || pin_in_pinmap(digitalPinToPinName(p), PinMap_I2C_SCL))
#define digitalPinHasPWM(p) (pin_in_pinmap(digitalPinToPinName(p), PinMap_TIM))
#define digitalPinHasSerial(p) \
    (pin_in_pinmap(digitalPinToPinName(p), PinMap_UART_RX) || pin_in_pinmap(digitalPinToPinName(p), PinMap_UART_TX))
#define digitalPinHasSPI(p)                                    \
    (pin_in_pinmap(digitalPinToPinName(p), PinMap_SPI_MOSI) || \
     pin_in_pinmap(digitalPinToPinName(p), PinMap_SPI_MISO) || \
     pin_in_pinmap(digitalPinToPinName(p), PinMap_SPI_SCLK) || pin_in_pinmap(digitalPinToPinName(p), PinMap_SPI_SSEL))
...
#define pinIsSerial(p)                                                             \
    ((digitalPinToPinName(p) == digitalPinToPinName(PIN_SERIAL_RX & PNUM_MASK)) || \
     (digitalPinToPinName(p) == digitalPinToPinName(PIN_SERIAL_TX & PNUM_MASK)))
#endif
...
_Static_assert((PWM_RESOLUTION > 0) && (PWM_RESOLUTION <= 32), "PWM_RESOLUTION must be 0 < x <= 32!");
