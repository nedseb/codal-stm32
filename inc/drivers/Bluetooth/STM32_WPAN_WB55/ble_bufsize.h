 ...
 */
#define BLE_DEFAULT_PREP_WRITE_LIST_SIZE BLE_PREP_WRITE_X_ATT(BLE_DEFAULT_MAX_ATT_SIZE)
 ...
 */
#define BLE_MBLOCKS_CALC(pw, mtu, n_link) ((pw) + MAX(BLE_MEM_BLOCK_X_MTU(mtu, n_link), BLE_MBLOCKS_SECURE_CONNECTIONS))
 ...
 */
#if (BEACON_ONLY != 0)
    #define BLE_FIXED_BUFFER_SIZE_BYTES 4'100 /* Beacon only */
#elif (LL_ONLY_BASIC != 0)
    #define BLE_FIXED_BUFFER_SIZE_BYTES 6'040 /* LL only Basic*/
#elif (LL_ONLY != 0)
    #define BLE_FIXED_BUFFER_SIZE_BYTES 6'288 /* LL only Full */
#elif (SLAVE_ONLY != 0)
    #define BLE_FIXED_BUFFER_SIZE_BYTES 6'408 /* Peripheral only */
#elif (BASIC_FEATURES != 0)
    #define BLE_FIXED_BUFFER_SIZE_BYTES 7'184 /* Basic Features */
#else
    #define BLE_FIXED_BUFFER_SIZE_BYTES 7'468 /* Full stack */
#endif
...
#define BLE_TOTAL_BUFFER_SIZE(n_link, mblocks_count)                           \
    (16 + BLE_FIXED_BUFFER_SIZE_BYTES + (BLE_PER_LINK_SIZE_BYTES * (n_link)) + \
 ...
 */
#define BLE_EXT_ADV_BUFFER_SIZE(set_nbr, data_len) (2'512 + ((892 + (DIVC(data_len, 207) * 244)) * (set_nbr)))
...
#define BLE_TOTAL_BUFFER_SIZE_GATT(num_gatt_attributes, num_gatt_services, att_value_array_size) \
    (((((att_value_array_size) - 1) | 3) + 1) + (40 * (num_gatt_attributes)) + (48 * (num_gatt_services)))
