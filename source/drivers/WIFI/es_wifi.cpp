    ...
    while (ptr != NULL) {
        switch (num++) {
            ...
            case 1:
                ...
                AP->SSID[ES_WIFI_MAX_SSID_NAME_SIZE] = '\0';
                break;
...
static ES_WIFI_Status_t AT_RequestSendData(ES_WIFIObject_t* Obj, uint8_t* cmd, uint8_t* pcmd_data, uint16_t len,
                                           uint8_t* pdata)
{
    int16_t send_len = 0;
    int16_t recv_len = 0;
 ...
 * @retval Operation Status.
 */
static ES_WIFI_Status_t AT_RequestReceiveData(ES_WIFIObject_t* Obj, uint8_t* cmd, char* pdata, uint16_t Reqlen,
                                              uint16_t* ReadData)
{
    int len;
 ...
 * @retval Operation Status.
 */
ES_WIFI_Status_t ES_WIFI_RegisterBusIO(ES_WIFIObject_t* Obj, IO_Init_Func IO_Init, IO_DeInit_Func IO_DeInit,
                                       IO_Delay_Func IO_Delay, IO_Send_Func IO_Send, IO_Receive_Func IO_Receive)
{
                              ...
                              int32_t result[])
{
    ...
    sprintf((char*)Obj->CmdData, "T1=%d.%d.%d.%d\r", address[0], address[1], address[2], address[3]);
    ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
...
ES_WIFI_Status_t ES_WIFI_StartClientConnection(ES_WIFIObject_t* Obj, ES_WIFI_Conn_t* conn)
{
    ...
    if ((ret == ES_WIFI_STATUS_OK) &&
        ((conn->Type == ES_WIFI_TCP_CONNECTION) || (conn->Type == ES_WIFI_TCP_SSL_CONNECTION))) {
    ...
    if ((ret == ES_WIFI_STATUS_OK) &&
        ((conn->Type == ES_WIFI_TCP_CONNECTION) || (conn->Type == ES_WIFI_TCP_SSL_CONNECTION))) {
        sprintf((char*)Obj->CmdData, "P3=%d.%d.%d.%d\r", conn->RemoteIP[0], conn->RemoteIP[1], conn->RemoteIP[2],
                conn->RemoteIP[3]);
        ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
    }
 ...
 * @retval Operation Status.
 */
ES_WIFI_Status_t ES_WIFI_SendData(ES_WIFIObject_t* Obj, uint8_t Socket, uint8_t* pdata, uint16_t Reqlen,
...
ES_WIFI_Status_t ES_WIFI_SendDataTo(ES_WIFIObject_t* Obj, uint8_t Socket, uint8_t* pdata, uint16_t Reqlen,
                                    uint16_t* SentLen, uint32_t Timeout, uint8_t* IPaddr, uint16_t Port)
{
    uint32_t wkgTimeOut;
...
ES_WIFI_Status_t ES_WIFI_ReceiveDataFrom(ES_WIFIObject_t* Obj, uint8_t Socket, uint8_t* pdata, uint16_t Reqlen,
                                         uint16_t* Receivedlen, uint32_t Timeout, uint8_t* IPaddr, uint16_t* pPort)
{
    uint32_t wkgTimeOut;
...
ES_WIFI_Status_t ES_WIFI_StoreCreds(ES_WIFIObject_t* Obj, ES_WIFI_CredsFunction_t credsFunction, uint8_t credSet,
                                    uint8_t* ca, uint16_t caLength, uint8_t* certificate, uint16_t certificateLength,
...
ES_WIFI_Status_t ES_WIFI_StoreCA(ES_WIFIObject_t* Obj, ES_WIFI_CredsFunction_t credsFunction, uint8_t credSet,
...
ES_WIFI_Status_t ES_WIFI_StoreCertificate(ES_WIFIObject_t* Obj, ES_WIFI_CredsFunction_t credsFunction, uint8_t credSet,
...
ES_WIFI_Status_t ES_WIFI_StoreKey(ES_WIFIObject_t* Obj, ES_WIFI_CredsFunction_t credsFunction, uint8_t credSet,
                                  uint8_t* key, uint16_t keyLength)
{
    ES_WIFI_Status_t ret = ES_WIFI_STATUS_ERROR;
    LOCK_WIFI();
